...
export default function RoomCodePanel({
  ...
  onShowLargeCode: () => void
  onCopyInvite: () => void
}) {
  if (roomStatus === 'waiting') {
    return (
              <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
                <div className="rounded-3xl bg-gradient-to-br from-sky-400 via-sky-500 to-cyan-500 p-6 font-bitbit text-white shadow-xl shadow-sky-200">
                  <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-black text-sky-50 [paint-order:stroke_fill] [-webkit-text-stroke:1.5px_#000]">참가코드</p>
                      <div
                        className="mt-1 text-6xl font-black tracking-wider [paint-order:stroke_fill] [-webkit-text-stroke:2px_#000]"
                        style={{ textShadow: '0 4px 0 rgba(0,0,0,0.35), 0 8px 16px rgba(0,0,0,0.2)' }}
                      >
                        {roomCode}
                      </div>
                    </div>
  ...
  return (
              <div className="mx-auto flex w-full max-w-3xl flex-wrap items-center justify-center gap-5 rounded-3xl border border-sky-100 bg-white px-6 py-5 text-center shadow-xl shadow-sky-100">
                ...
                <div className="flex flex-col items-center gap-1">
                  ...
                  <p className="text-[11px] font-bold text-slate-400">누르면 크게 보기</p>
                </div>
              </div>
  )
}
