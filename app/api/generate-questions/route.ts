...
export async function POST(request: NextRequest) {
  try {
    ...
    // 유형별 생성 개수 {"CHOICE":3,"OX":1,"SHORT":1} — 있으면 유형과 개수를 정확히 지정.
    let typeCounts: Partial<Record<ValidType, number>> | undefined
    let allowedTypes: ValidType[] | undefined
    const rawCounts = formData.get('questionCounts')
    if (typeof rawCounts === 'string' && rawCounts.trim()) {
      try {
        const parsed = JSON.parse(rawCounts) as Record<string, unknown>
        const collected: Partial<Record<ValidType, number>> = {}
        for (const [k, v] of Object.entries(parsed)) {
          if (VALID_TYPES.includes(k as ValidType)) {
            const n = Math.max(0, Math.min(MAX_QUESTION_COUNT, Math.floor(Number(v) || 0)))
            if (n > 0) collected[k as ValidType] = n
          }
        }
        if (Object.keys(collected).length > 0) {
          typeCounts = collected
          allowedTypes = Object.keys(collected) as ValidType[]
        }
      } catch {
        // 형식이 잘못되면 무시하고 자유 출제
      }
    ...
    // 하위호환: 유형만 담긴 questionTypes 배열도 계속 지원
    if (!allowedTypes) {
      ...
      if (typeof rawTypes === 'string' && rawTypes.trim()) {
        try {
          ...
          if (Array.isArray(parsed)) {
            const filtered = parsed.filter((t): t is ValidType => VALID_TYPES.includes(t))
